...
export default function Special({ size, specifics, num }) {
  const currentStage = useSelector(selectCurrentStage);
  const { position } = useSelector(selectMapState)[currentStage].special[num];
  ...
  const isVisible = useSelector(selectSpecialItem)[num];
