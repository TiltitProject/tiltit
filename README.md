...
- 🛠️ Optimization
  + 한번에 너무 많은 컴포넌트를 렌더링 시켰었다.
  + 물리엔진에 전달되는 컴포넌트는 많은 cost가 소요된다.
- 🎞️ User Experience
  + Gyroscope 원점 설정 및 미리보기 기능
  + 애니메이션 효과
    * 화면 전환
    * 제한 시간
    * 점수 집계
...
<img width="500" alt="image" src="https://github.com/TiltitProject/tiltit/assets/106131005/424d21e6-1cd7-4ac5-a84b-9b8801991114">
...
<br>
위의 구글 스프레드시트의 데이터를 json 데이터로 추출하면 아래와 같습니다.
<br>
...
위의 json 데이터에는 column 별 row의 entity 데이터가 객체 형식으로 있습니다. 하지만 제 프로젝트에서 필요한 데이터는 entity 별 col, row의 값이었으므로, json 정보를 가공해 줄 함수를 만들었습니다.
...
```
이러한 entity의 col, row 값에 grid와 넓이(필요시)의 값을 적용하는 함수를 통해 아래와 같이 선언적으로 entity들의 좌표와 크기를 구할 수 있었습니다.
<br>
...
```
기존 방식 대로라면 30개에서 많게는 100개 까지 되는 entity들의 위치와 크기를 하나씩 정의해줘야 했겠지만,
구글 스프레드시트에 디자인한 entity들의 위치와 크기를 함수 호출 하나로 정의해 줄 수 있게 되었습니다.
...
# 🎞️ User Experience
지금까지 해왔던 웹페이지 개발과는 달리, 게임의 가장 큰 목적은 유저의 즐거운 플레이 경험이라고 생각합니다.
그러다보니, 웹페이지를 개발했을 때 보다 유저 경험을 더욱 신경 쓸 수 밖에 없었습니다.
...
기존과 비교해보면 화면전환이 더욱 생동감 있고, 공간적 이동을 연상시키는 것을 확인할 수 있습니다.

### 2.2 제한 시간
---
게임의 화면이 작다고 하더라도 실제 게임을 하는 유저는 플레이중인 캐릭터에 집중하게 됩니다. 하지만 유저가 게임중이라 하더라도, 게임의 제한 시간이 임박했다는 정보는 효과적이고 확실하게 전달할 필요가 있었습니다.
