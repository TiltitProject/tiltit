...
# Refactoring
리팩터링의 기준은 크게 두가지로 가져갈 예정입니다.


1. 이해하기 쉽고 재사용성 높은 코드
게임에서 발생하는 다양한 상호작용을 명령적인 코드로 구현했습니다.

하지만 명령적인 코드들의 나열은 가독성을 해치며, 의미가 불분명합니다.
명령적인 코드를 수행하는 기능과 의미 단위로 나누어 이해하기 쉽도록 개선 할 예정입니다.

또한, 반복되는 코드는 추상화하여 재사용성을 높일 예정입니다

2. 반복문에 함수형 프로그래밍 적용
인프런의 `함수형 프로그래밍과 JavaScript ES6+`, `함수형 프로그래밍과 JavaScript ES6+ 응용편`을 통해 함수형 프로그래밍을 학습했습니다.

프로젝트에 함수형 프로그래밍을 적용하면 다음과 같은 이점이 있을 것이라 생각합니다. fxjs 라이브러리를 활용했습니다.
- 복잡한 로직을 간결한 코드로 해결
- 자바스크립트의 이터러블/이터레이터 프로토콜을 활용한 선언적 리스트 순회
