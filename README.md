...
- [🛠️ Optimization](#-optimization)
  * [1. 한 번에 너무 많은 컴포넌트를 렌더링 시켰다.](#1-한-번에-너무-많은-컴포넌트를-렌더링-시켰다)
  * [2. 물리엔진에 전달되는 컴포넌트는 많은 리소스가 소요된다.](#2-물리엔진에-전달되는-컴포넌트는-많은-리소스가-소요된다)
...
---
하드코딩으로 맵을 만들다 보니, GUI 환경에서 맵을 만들 수 있다면 가장 이상적일 것 같았습니다. 그렇다고 게임을 만드는 GUI 툴을 만드는 건, 완전히 다른 프로젝트가 될 것 같았습니다.
...
가장 먼저, 보라색 테두리의 이동 플래그(f1)에 도달했을 때를 기준으로 물리엔진에 두 번째 뷰포트의 entity를 전달하는 방식을 고안했습니다.
...
물리엔진은 초당 60회 entity들을 업데이트합니다. 그렇기 때문에 캐릭터는 벽에 가로막히는 등, 물리엔진에 정의해 놓은 물리법칙에 적용되며 상호작용할 수 있습니다.

하지만 초당 60회의 업데이트는 많은 리소스를 필요로 합니다. 첫 번째 최적화를 통해 entity를 30개가량 줄였지만, 여전히 약 70개의 entity들이 물리엔진에 전달되고 있었고, 초당 60회 entity들을 업데이트하고 있었습니다. 물리엔진에 전달되는 entity를 줄일 방법이 필요했습니다.

<img width="300" alt="image" src="https://github.com/TiltitProject/tiltit/assets/106131005/91f508a3-9163-431b-8a45-7c495d128fd5">

상호작용이 비교적 간단한 아이템과 같은 entity들은 물리엔진에 건네지 않고 상태로 관리하기로 했습니다.
...
아래와 같이 Custom Thunk를 활용하여 이미 획득한 아이팀인지 상태를 확인 후 dispatch 하도록 하였습니다.
